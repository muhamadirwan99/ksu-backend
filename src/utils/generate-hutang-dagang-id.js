...
const generateHutangDagangId = async (tg_pembelian) => {
  ...
  while (idExists) {
    ...
    hutangDagangId = `HD-${datePrefix}${increment}`;

    // Cek apakah ID sudah ada di database
    const existingPurchase = await prismaClient.hutangDagang.findUnique({
      where: {
        id_hutang_dagang: hutangDagangId,
      },
    });
  ...
  return hutangDagangId;
};

export { generateHutangDagangId };
